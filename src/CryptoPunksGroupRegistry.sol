...
contract CryptoPunksGroupRegistry is ICryptoPunksGroupRegistry, ERC1155, AccessControl, ReentrancyGuard {
    ...
    constructor(address cryptoPunksMarketAddress, address mosaicRegistryAddress) ERC1155("CryptoPunks Mosaic Ticket") {
    ...
    function create(uint256 targetPunkId, uint256 targetMaxPrice) external returns (uint192 groupId) {
        ++latestGroupId;
        ...
        emit GroupCreated(latestGroupId, msg.sender, targetMaxPrice, totalTicketSupply, unitTicketPrice);
        return latestGroupId;
    }

    function contribute(uint192 groupId, uint64 ticketQuantity) external payable onlyValidGroup(groupId) {
        Group storage group = groups[groupId];

        uint256 ticketsLeft = group.totalTicketSupply - group.ticketsBought;
        require(ticketQuantity <= ticketsLeft, "Fewer tickets remaining than requested");
    ...
    function buy(uint192 groupId) external nonReentrant onlyValidGroup(groupId) {
        Group storage group = groups[groupId];
        require(hasContribution(msg.sender, groupId), "Only ticket holders can initiate a buy");
        ...
        require(group.totalContribution >= offeredPrice, "Offered price is greater than the current contribution");
        cryptoPunksMarket.buyPunk{value: offeredPrice}(punkId);
        require(cryptoPunksMarket.punkIndexToAddress(punkId) == address(this), "Unexpected ownership");
    ...
    function finalizeOnWon(uint192 groupId) public onlyValidGroup(groupId) {
        ...
        require(address(mosaicRegistry) != address(0x0), "Exhibit registry must be set");

        // FIXME: Defend against reentrancy attacks in edge cases where the same cryptopunk ID is used later
        cryptoPunksMarket.transferPunk(address(mosaicRegistry), group.targetPunkId);
        group.originalId = mosaicRegistry.create(group.targetPunkId, group.ticketsBought);
    ...
    {
        Group storage group = groups[groupId];
        require(group.status == GroupStatus.Claimable, "The group is not finalized");
        require(
            hasContribution(msg.sender, groupId) || msg.sender == group.creator, "Only ticket holders can claim tokens"
        );
     ...
     */
    function refund(uint192 groupId, address payable contributor) external nonReentrant onlyValidGroup(groupId) {
        Group storage group = groups[groupId];
        require(group.status == GroupStatus.Claimable || group.expiry > block.timestamp, "The group is not finalized");
        require(refundableTickets[groupId][contributor] > 0, "Only refundable ticket holders can get refunds");
    ...
    function getRefundPerTicket(Group storage group) private view returns (uint256 refundPerTicket) {
    ...
    function getGroupInfo(uint192 groupId)
        ...
        view
        onlyValidGroup(groupId)
        returns (address creator, uint256 targetMaxPrice, uint96 ticketsBought, GroupStatus status)
    {
        Group storage group = groups[groupId];
        return (group.creator, group.targetMaxPrice, group.ticketsBought, group.status);
    }
    ...
    // Ticket
    //
    function getTickets(address inquired, uint192 groupId) public view returns (uint256) {
        return balanceOf(inquired, uint256(groupId));
    }

    function isCreator(address inquired, uint192 groupId) public view returns (bool) {
        return groups[groupId].creator == inquired;
    }

    function hasContribution(address inquired, uint192 groupId) public view returns (bool) {
    ...
    // Admin
    //
    function airdrop(address to, uint256[] memory ids, uint256[] memory amounts)
        external
