    ...
    ReentrancyGuardUpgradeable
{
    ...
    function forceLose(uint192 groupId) external onlyValidGroup onlyRole(CURATOR_ROLE) {
        if (getGroupLifeCycle(groupId) != GroupLifeCycle.Active) {
            revert NotActive();
        }
        CryptoPunksGroupStorage.get().forceLost[groupId] = true;
        CryptoPunksGroupStorage.get().groups[groupId].expiresAt = uint40(block.timestamp - 1);
    ...
    function getGroupLifeCycle(
        ...
        if (group.status == GroupStatus.Open) {
            if (group.expiresAt < block.timestamp || CryptoPunksGroupStorage.get().forceLost[groupId]) {
